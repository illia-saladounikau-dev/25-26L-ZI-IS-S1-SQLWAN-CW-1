...
SELECT DISTINCT
    pc.Name,
    MIN(p.ListPrice) OVER (PARTITION BY pc.Name) AS MinPrice,
    MAX(p.ListPrice) OVER (PARTITION BY pc.Name) AS MaxPrice,
    COUNT(*) OVER (PARTITION BY pc.Name) AS CategoryProductsCount
FROM SalesLT.Product p
JOIN SalesLT.ProductCategory pc ON p.ProductCategoryID = pc.ProductCategoryID;
-- =============================================
-- Zadanie 2
-- =============================================
-- Kod poniżej jest schematem edukacyjnym Oracle HR Database:
-- (źródło skryptu z ładowaniem danych dla T-SQL to https://raw.githubusercontent.com/dzikrymh/sql-hr-schema-mssql/refs/heads/master/hr-schema-mssql.sql)
...
-- Przypadek polega na przedstawieniu minimalnych, średnich oraz maksymalnych dla każdej pary kraj<->nazwa_pocyzji. Wyjściowe dane to: Nazwa_pozycji, nazwa_kraju, min_zarobki, śr_zarobki, max_zarobki

SELECT DISTINCT
    j.job_title AS Posistion,
    c.country_name AS Country,
    MIN(e.salary) OVER (PARTITION BY c.country_name, e.job_id) AS MinSalary,
    AVG(e.salary) OVER (PARTITION BY c.country_name, e.job_id) AS AverageSalary,
    MAX(e.salary) OVER (PARTITION BY c.country_name, e.job_id) AS MaxSalary
FROM Employees e
...
-- Zadanie 3
-- =============================================
-- Przypadek polega na danych z poprzedniego zadania, przedstawia liczby pracownikw według stanowiska z użyciem pivot
SELECT *
FROM
(
    SELECT 
...
(
    COUNT(employee_id)
    FOR job_title IN ([Programmer], [Accountant], [Sales Representative], [Stock Clerk])
) AS pvt;
...
UNPIVOT
(
    employee_count FOR job_title IN ([Programmer], [Accountant], [Sales Representative])
) AS unpvt;
-- =============================================
-- Zadanie 3
-- =============================================
-- Przypadek polega na danych z przedpoprzedniego zadania, przedstawia sumę wydatków na pensje dla każdego departamentu oraz łączną sumę
SELECT
    d.department_name,
    SUM(e.salary) AS SalariesExpensesSum
FROM employees e
JOIN departments d ON e.department_id = d.department_id
